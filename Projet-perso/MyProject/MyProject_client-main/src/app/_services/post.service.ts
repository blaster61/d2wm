...
export class PostService {
  ...
  createPost(title: string, content: string, author: string): Observable<Post> {
    return this.http.post<Post>(POST_API + "new", {title, content, author}, httpOptions);
  }
  ...
  updatePost(id: string, title: string, content: string): Observable<Post> {
    return this.http.put<Post>(POST_API + "update/" + id, {title, content}, httpOptions);
  }
