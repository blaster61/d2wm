...
export class EditUserComponent {
  ...
  onSubmit(): void {
    const { login, isAdmin, image } = this.form;
    
    this.formData = new FormData()

    this.formData.append('login',login)
    this.formData.append('isAdmin',isAdmin)
    this.formData.append('image',image)
    
    this.userService.updateUser(this.formData, this.id).subscribe(
      data => {
        console.log(data);
